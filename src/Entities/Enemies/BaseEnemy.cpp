...
BaseEnemy::BaseEnemy(Vec2 & pos, double lifemax, double damage)
    : DamageableEntity(pos, lifemax), damage(damage),
    speed(2.0), lastDamage(0), cooldownDamage(0.0), timerHitDamage(0.0),
      imageEnemyHurt("../assets/enemy1_hurt.png"),
      imageEnemy("../assets/enemy1.png")
    {}
...
void BaseEnemy::draw() {
    ...
    if(this->timerHitDamage > 0.0){
        ...
        Renderer::getInstance().drawImage(imageEnemyHurt, this->getPos(), Vec2(60.0, 60.0), rota);
        ...
        Renderer::getInstance().drawImage(imageEnemy, this->getPos(), Vec2(60.0, 60.0), rota);
    }
}
